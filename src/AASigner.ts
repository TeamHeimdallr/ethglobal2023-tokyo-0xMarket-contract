...
export class AAProvider extends BaseProvider {
  ...
  constructor(entryPointAddress: string, provider: Provider) {
    ...
    this.entryPoint = IEntryPoint__factory.connect(entryPointAddress, provider);
  }
}
...
export class AASigner extends Signer {
  _account?: TradableAccount;

  private _isPhantom = true;
  public entryPoint: IEntryPoint;
  ...
  constructor(
    ...
    super();
    this.entryPoint = IEntryPoint__factory.connect(entryPointAddress, signer);
  }
  ...
  async connectAccountAddress(address: string): Promise<void> {
    ...
    this._account = TradableAccount__factory.connect(address, this.signer);
  ...
  async getAccount(): Promise<TradableAccount> {
    await this.syncAccount();
